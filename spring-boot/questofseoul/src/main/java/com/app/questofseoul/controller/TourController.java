...
public class TourController {
    ...
    @GetMapping("/tours")
    public ResponseEntity<List<TourListItem>> listTours() {
        UUID userId = getUserIdOrNull();
        return ResponseEntity.ok(tourDetailService.getTourList(userId));
    }
    ...
    @Operation(summary = "Run 처리", description = "mode: START | CONTINUE. 유저당 투어별 IN_PROGRESS 1개")
    @SecurityRequirement(name = "bearerAuth")
    @PostMapping("/tours/{tourId}/runs")
    public ResponseEntity<RunResponse> handleRun(
            @PathVariable Long tourId,
            @RequestBody @Valid RunRequest body) {
        UUID userId = authService.getCurrentUserId();
        RunResponse response = tourRunService.handleRun(userId, tourId, body.getMode());
