...
public interface UserSpotProgressRepository extends JpaRepository<UserSpotProgress, Long> {
    ...
    @Query("SELECT p.spot.id FROM UserSpotProgress p WHERE p.tourRun.id = :runId AND p.progressStatus = :status")
    List<Long> findCompletedSpotIdsByTourRunId(Long runId, ProgressStatus status);

    @Query("SELECT p FROM UserSpotProgress p JOIN FETCH p.spot s JOIN FETCH p.tourRun r JOIN FETCH r.tour WHERE r.user.id = :userId AND p.progressStatus <> com.app.questofseoul.domain.enums.ProgressStatus.PENDING AND s.type IN ('MAIN','SUB') AND (:tourId IS NULL OR r.tour.id = :tourId)")
